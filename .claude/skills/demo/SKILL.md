...
1. **Rebuild the demo repo.** `node scripts/make-demo.ts`. This recreates `demo/` (before/after C# fixture commits on `main` and `feat/queue-notifications`) without a manifest.
...
3. **Verify from the outside.** Run `node .claude/skills/chapter-review/validate.ts demo/.git/chapter-review/chapters.json` yourself and confirm it passes; do not trust the sub-agent's claim alone. Then confirm the write hit the demo repo, not the outer one: check the path `write` reported is under `demo/.git`, or that the demo manifest's `head` is `feat/queue-notifications`. A wrong-cwd `chapter-review write` lands in the outer repo's `.git/chapter-review/chapters.json` instead — overwriting this project's own dogfood manifest if one is present, and silently producing a manifest for the wrong branch either way.

4. **Launch the dev host.** Always run `npm run build` in `extension/` first — don't gate it on `extension/out/extension.js` or `extension/skill/` existing, because staleness is invisible to an existence check and a stale build is the failure mode here. `compile` wipes `out/` before `tsc`, which matters: `tsc` never deletes orphaned output, and an orphan left by a module split (e.g. an old `out/tree.js` after `src/tree.ts` became `src/tree/*.ts`) shadows the new directory in Node's resolution and loads dead code against the current API. The symptom is a method-not-found throw in the tree provider and an empty Chapters view that looks like a missing manifest. Then `code --new-window --extensionDevelopmentPath="<repo-root>/extension" "<repo-root>/demo"`. Tell the user: click the "Chapter Review" icon in the activity bar → "Chapters" view.
...
- `node scripts/make-demo.ts --manifest` writes a scripted reference manifest instead; useful to compare against the skill's output.
